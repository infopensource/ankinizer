...
class Translator:
    def __init__(self, api_key='yKUxbcpmj8mtR', base_url='https://ai.liaobots.work/v1'):
        self.client = OpenAI(
            ...
            messages=[
                {
                    ...
                    "content": f'''
                        ...
                        格式要求：单词+半角分号+词性(名词一律分为阴性阳性中性三种)和释义(如果这个词经过了变位则同时输出原型，这个词已经是原型则不再次输出；如果这个单词没有经过变位但是这个单词的变位不规则，输出变位置表，要求变位表用html的表格标签输出。同理，名词复数有特殊变形的也按照如上方法处理，形容词亦然)+<br>+例句（例句的中文翻译）+<br>+构词法或词源助记（不要凭空捏造牵强附会的解释）。
                        按照格式输出，不要有多余的内容。
                    '''
                }
            ],
